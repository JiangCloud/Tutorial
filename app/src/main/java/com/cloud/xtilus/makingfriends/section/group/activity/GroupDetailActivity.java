...
import com.cloud.xtilus.makingfriends.common.widget.SwitchItemView;
import com.cloud.xtilus.makingfriends.http.OkHttpHelper;
import com.cloud.xtilus.makingfriends.http.RequestCallback;
...
import com.cloud.xtilus.makingfriends.section.dialog.EditTextDialogFragment;
...
import com.cloud.xtilus.makingfriends.section.group.fragment.GroupEditFragment;
import com.cloud.xtilus.makingfriends.section.group.viewmodels.GroupDetailViewModel;
import com.cloud.xtilus.makingfriends.section.search.SearchGroupChatActivity;
...
public class GroupDetailActivity extends BaseInitActivity implements EaseTitleBar.OnBackPressListener, View.OnClickListener, SwitchItemView.OnCheckedChangeListener {
    ...
    private void initGroupView() {
        ...
        EaseUser easeUser= DemoHelper.getInstance().getUserInfoByUserName(groupId);

        Glide.with(mContext).load(easeUser.getAvatar())
                .apply(RequestOptions.placeholderOf(R.drawable.ease_group_icon)
                        .diskCacheStrategy(DiskCacheStrategy.ALL)).into(ivGroupAvatar);
                ...
                .setConfirmClickListener(new EditTextDialogFragment.ConfirmClickListener() {
                    ...
                    public void onConfirmClick(View view, String content) {
                        if(!TextUtils.isEmpty(content)) {
                            ...
                            params.put("groupName", content);
                            OkHttpHelper.getInstance().post(Constant.URL_UPDATE_updateGroupName,params , new RequestCallback<ResponseBean>(){


                                @Override
                                public void onSuccess(Response response, ResponseBean responseBean) {
                                    Log.d("Ret_code","============="+responseBean.getRet_code());
                                    if(responseBean.getRet_code()==200){
                                        try {
                                            JSONObject data=new JSONObject();
                                            data.put("groupName",content);
                                            //data.put("")
                                            ...
                                            String action="6000";//action可以自定义
                                            cmdMsg.setChatType(EMMessage.ChatType.GroupChat);
                                            EMCmdMessageBody cmdBody = new EMCmdMessageBody(action);
                                            cmdMsg.setTo(groupId);
                                            cmdMsg.addBody(cmdBody);
                                            cmdMsg.setAttribute("groupInfo",jsonObject.toString());
                                            cmdMsg.setMessageStatusCallback(new EMCallBack() {
                                                @Override
                                                public void onSuccess() {
                                                    DemoHelper.getInstance().updateGroupName(groupId,content);
        ...
        GroupEditFragment.showDialog(mContext,
                ...
                new GroupEditFragment.OnSaveClickListener() {
                    ...
                    public void onSaveClick(View view, String content) {
                        ...
                        String atUser=AT_PREFIX + "所有成员" + AT_SUFFIX;
...
*/
                        ...
                        EMMessage txtMsg = EMMessage.createTxtSendMessage(atUser+getString(R.string.demo_group_listener_onAnnouncementChanged_content, content) ,groupId);
                        txtMsg.setChatType(EMMessage.ChatType.GroupChat);
                        txtMsg.setFrom(DemoHelper.getInstance().getCurrentUser());
                        txtMsg.setAttribute(EaseConstant.MESSAGE_ATTR_AT_MSG, EaseConstant.MESSAGE_ATTR_VALUE_AT_MSG_ALL);
                        EMClient.getInstance().chatManager().sendMessage(txtMsg);


                        txtMsg.setMessageStatusCallback(new EMCallBack() {
                            @Override
                            public void onSuccess() {
                                Log.d("cmdMsg","txtMsg sent to susess====================");
                            }
