...
import com.cloud.xtilus.makingfriends.section.search.SearchSingleChatActivity;
import com.hyphenate.chat.EMClient;
import com.hyphenate.chat.EMConversation;
import com.cloud.xtilus.makingfriends.R;
import com.cloud.xtilus.makingfriends.section.chat.viewmodel.ChatViewModel;
...
public class SingleChatSetActivity extends BaseInitActivity implements EaseTitleBar.OnBackPressListener, View.OnClickListener, SwitchItemView.OnCheckedChangeListener {
    ...
    protected void initData() {
        ...
        itemUserInfo.getAvatar().setShapeType(1);
         ...
         easeUser=DemoHelper.getInstance().getUserInfoByUserName(toChatUsername);


        if(easeUser!=null&&!TextUtils.isEmpty(easeUser.getNickname())){
            itemUserInfo.getTvTitle().setText(easeUser.getNickname());
        }else {
        ...
        Glide.with(mContext).load(easeUser.getAvatar())
                .apply(RequestOptions.placeholderOf(R.drawable.ease_default_avatar)
                        .diskCacheStrategy(DiskCacheStrategy.ALL)).into( itemUserInfo.getAvatar());
        ...
        switch (v.getId()) {
            case R.id.item_user_info :
               ...
               // user.setUsername(toChatUsername);
                ContactDetailActivity.actionStart(mContext, easeUser);
                break;
