...
import com.cloud.xtilus.makingfriends.widget.PhotoViewPager;
...
public class BigImageActivity extends BaseActivity {
    ...
    private class MyAdapter extends PagerAdapter {
        ...
        public PhotoView instantiateItem(ViewGroup container, int position) {
            ...
            if(!isNetUrl){
                Glide.with(BigImageActivity.this).load(image).diskCacheStrategy(DiskCacheStrategy.ALL).placeholder(R.mipmap.default_image).error(R.mipmap.default_image).into(photoView);
            }
            if(isNetUrl&&!image.contains("http")){
                image= Constant.baseImgUrl+image;
            }
            Glide.with(BigImageActivity.this).load(image).diskCacheStrategy(DiskCacheStrategy.ALL).placeholder(R.mipmap.default_image).error(R.mipmap.default_image).into(photoView);
