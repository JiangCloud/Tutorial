package com.cloud.xtilus.makingfriends.section.conference;
...
import com.cloud.xtilus.makingfriends.common.utils.PreferenceManager;
import com.cloud.xtilus.makingfriends.common.widget.DebugPanelView;
import com.cloud.xtilus.makingfriends.common.widget.EasePageIndicator;
import com.cloud.xtilus.makingfriends.common.widget.conference.ConferenceMemberView;
import com.cloud.xtilus.makingfriends.common.widget.conference.IncomingCallView;
import com.cloud.xtilus.makingfriends.common.widget.conference.MemberViewGroup;
...
public class ConferenceActivity extends BaseActivity implements EMConferenceListener {
    ...
    public static void startConferenceCall(Context context, String groupId) {
        ...
        i.putExtra(DemoConstant.EXTRA_CONFERENCE_IS_CREATOR, true);
        i.putExtra(DemoConstant.EXTRA_CONFERENCE_GROUP_ID, groupId);
    ...
    public static void receiveConferenceCall(Context context, String conferenceId, String password, String inviter, String groupId) {
        Intent i = new Intent(context, ConferenceActivity.class);
        i.putExtra(DemoConstant.EXTRA_CONFERENCE_ID, conferenceId);
        i.putExtra(DemoConstant.EXTRA_CONFERENCE_PASS, password);
        i.putExtra(DemoConstant.EXTRA_CONFERENCE_INVITER, inviter);
        i.putExtra(DemoConstant.EXTRA_CONFERENCE_IS_CREATOR, false);
        i.putExtra(DemoConstant.EXTRA_CONFERENCE_GROUP_ID, groupId);
        i.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        context.startActivity(i);
    }
    ...
    private void checkSaveInstanceState(Bundle bundle) {
        if(bundle != null) {
            confId = bundle.getString(DemoConstant.EXTRA_CONFERENCE_ID);
            password = bundle.getString(DemoConstant.EXTRA_CONFERENCE_PASS);
        }
    }
    ...
    protected void onNewIntent(Intent intent) {
        ...
        /**
         * 为了在{@link UserActivityLifecycleCallbacks#restartSingleInstanceActivity(Activity)}
    ...
    private void init() {
        ...
        if(!TextUtils.isEmpty(confId)) {
            ...
            groupId = getIntent().getStringExtra(DemoConstant.EXTRA_CONFERENCE_GROUP_ID);
            isCreator = getIntent().getBooleanExtra(DemoConstant.EXTRA_CONFERENCE_IS_CREATOR, false);
            if (isCreator) {
                incomingCallView.setVisibility(View.GONE);
                selectUserToJoinConference();
            } else {
                confId = getIntent().getStringExtra(DemoConstant.EXTRA_CONFERENCE_ID);
                password = getIntent().getStringExtra(DemoConstant.EXTRA_CONFERENCE_PASS);

                initLocalConferenceView();
                String inviter = getIntent().getStringExtra(DemoConstant.EXTRA_CONFERENCE_INVITER);
    ...
    private void inviteUserToJoinConference(String[] contacts) {
        try {
            ...
            object.put(DemoConstant.EXTRA_CONFERENCE_INVITER, EMClient.getInstance().getCurrentUser());
            object.put(DemoConstant.EXTRA_CONFERENCE_GROUP_ID, groupId);
    ...
    private void sendInviteMessage(String to, String extension) {
        ...
        message.setAttribute(DemoConstant.MSG_ATTR_CONF_ID, conference.getConferenceId());
        message.setAttribute(DemoConstant.MSG_ATTR_CONF_PASS, conference.getPassword());
        // 扩展字段对于音视频会议不是必须的,只是增加了额外用于显示或者判断音视频会议类型的信息.
        message.setAttribute(DemoConstant.MSG_ATTR_EXTENSION, extension);
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        // 保留非正常退出的会议相关信息
        if(conference != null) {
            outState.putString(DemoConstant.EXTRA_CONFERENCE_ID, conference.getConferenceId());
            outState.putString(DemoConstant.EXTRA_CONFERENCE_PASS, conference.getPassword());
        }
    ...
    protected void onStop() {
        super.onStop();
        // 用于判断app是否进入后台
        if(!isFinishing() && !MakingFriendApplication.getInstance().getLifecycleCallbacks().isOnForeground()) {
            showFloatWindow();
        }
    }
    ...
    public void finish() {
        super.finish();
        MakingFriendApplication.getInstance().getLifecycleCallbacks().makeMainTaskToFront(this);
    }
