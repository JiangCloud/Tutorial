package com.cloud.xtilus.makingfriends.section.chat.activity;
...
public class LiveActivity extends BaseActivity implements EMConferenceListener {
    ...
    private void init() {
        ...
        boolean isCreator = getIntent().getBooleanExtra(DemoConstant.EXTRA_CONFERENCE_IS_CREATOR, false);
        ...
        if (isCreator) {
            ...
            inviter = getIntent().getStringExtra(DemoConstant.EXTRA_CONFERENCE_INVITER);
                ...
                case R.id.btn_request_connect: // 申请视频连麦
                    ...
                    if (btnState == STATE_AUDIENCE) { // 当前按钮状态是观众，需要变成主播
                        if (currentRole == EMConferenceManager.EMConferenceRole.Audience) { // 发送消息，申请上麦
                            ...
                            sendRequestMessage(content, inviter, DemoConstant.OP_REQUEST_TOBE_SPEAKER);
                        } else { // 已经是主播，直接推流
                            publish();
                        ...
                        if (currentRole == EMConferenceManager.EMConferenceRole.Talker) { // 申请下麦
                            ...
                            sendRequestMessage(content, inviter, DemoConstant.OP_REQUEST_TOBE_AUDIENCE);
                        }
                    }
                    break;
    ...
    private MemberViewGroup.OnScreenModeChangeListener onScreenModeChangeListener = new MemberViewGroup.OnScreenModeChangeListener() {
        ...
        public void onScreenModeChange(boolean isFullScreenMode, @Nullable View fullScreenView) {
            if (isFullScreenMode) { // 全屏模式
                toolsPanelView.setBackgroundColor(getResources().getColor(R.color.transparent));
    ...
    private void sendInviteMessage(String to, boolean isGroupChat) {
        ...
        message.setAttribute(DemoConstant.EM_CONFERENCE_OP, DemoConstant.OP_INVITE);
        message.setAttribute(DemoConstant.EM_CONFERENCE_ID, conference.getConferenceId());
        message.setAttribute(DemoConstant.EM_CONFERENCE_PASSWORD, conference.getPassword());
        message.setAttribute(DemoConstant.EM_CONFERENCE_TYPE, conference.getConferenceType().code);
        if (isGroupChat) {
...
//        message.setAttribute(Constant.MSG_ATTR_EXTENSION, extension);
    ...
    private void sendRequestMessage(String content, String to, String op) {
        ...
        message.setAttribute(DemoConstant.EM_CONFERENCE_OP, op);
        message.setAttribute(DemoConstant.EM_CONFERENCE_ID, conference.getConferenceId());
        message.setAttribute(DemoConstant.EM_CONFERENCE_PASSWORD, conference.getPassword());
        message.setAttribute(DemoConstant.EM_MEMBER_NAME, EasyUtils.getMediaRequestUid(EMClient.getInstance().getOptions().getAppKey(), EMClient.getInstance().getCurrentUser()));
        message.setMessageStatusCallback(new EMCallBack() {
            @Override
            public void onSuccess() {
                EMLog.d(TAG, "Invite join conference success");
        ...
        EMMessageListener messageListener = new EMMessageListener() {
            ...
            public void onMessageReceived(List<EMMessage> messages) {
                for (EMMessage msg : messages) {
                    String op = msg.getStringAttribute(DemoConstant.EM_CONFERENCE_OP, "");
                    if (DemoConstant.OP_REQUEST_TOBE_SPEAKER.equals(op)) {
                        // 把该申请上线消息从会话中删除
                        ...
                        final String jid = msg.getStringAttribute(DemoConstant.EM_MEMBER_NAME, "");
                    ...
                    } else if (DemoConstant.OP_REQUEST_TOBE_AUDIENCE.equals(op)) {
                        // 把该申请下线消息从会话中删除
                        EMConversation conversation = EMClient.getInstance().chatManager().getConversation(msg.getFrom(), EMConversation.EMConversationType.Chat, true);
                        conversation.removeMessage(msg.getMsgId());

                        String jid = msg.getStringAttribute(DemoConstant.EM_MEMBER_NAME, "");
                        // changeRole.
                        EMClient.getInstance().conferenceManager().grantRole(conference.getConferenceId()
                                , new EMConferenceMember(jid, null, null)
