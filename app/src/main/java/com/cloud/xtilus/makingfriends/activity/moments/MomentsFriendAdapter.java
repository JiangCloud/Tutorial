...
public class MomentsFriendAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (position == 0) {
            ...
            Glide.with(context).load(avatar).diskCacheStrategy(DiskCacheStrategy.ALL).placeholder(R.mipmap.default_avatar).into(iv_avatar);
            iv_moment_bg = (ImageView) view.findViewById(R.id.iv_moment_bg);
            if (backgroudMoment!=null && backgroudMoment.size()!=0){
                Glide.with(context).load(backgroudMoment.get(0)).centerCrop().diskCacheStrategy(DiskCacheStrategy.ALL).placeholder(R.mipmap.bg_moments_header).error(R.mipmap.bg_moments_header).into(iv_moment_bg);
            }
            ...
            String rel_time = json.getString("publishTime");
            // 设置文章中的图片
            if (json.containsKey("imageStr")) {
                ...
                if (!TextUtils.isEmpty(imageStr)) {
                    ...
                    holder.image_1.setImageURI(Uri.parse(Constant.baseImgUrl
                            + images[0] + Constant.baseImgUrl_set));
            ...
            // 显示时间
            String serviceTime = DateHelper.getStringTime(System.currentTimeMillis());
