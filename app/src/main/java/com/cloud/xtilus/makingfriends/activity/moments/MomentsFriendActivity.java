...
public class MomentsFriendActivity extends BaseActivity implements SwipyRefreshLayout.OnRefreshListener {
    ...
    protected void onCreate(Bundle arg0) {
        ...
        if (MakingFriendApplication.getInstance().getUsername().equals(userId)) {
            ...
            avatar = MakingFriendApplication.getInstance().getUserJson().getString(Constant.JSON_KEY_AVATAR);
...
//        params.add(new Param("isFriend", isFriend));
        new OkHttpUtils(MomentsFriendActivity.this).post(params, Constant.URL_SOCIAL_FRIEND, new OkHttpUtils.HttpCallBack() {
            @Override
            public void onResponse(JSONObject jsonObject) {
                ...
                switch (code) {
                    case 200:
                        ...
                        String background = jsonObject.getString("backgroundImage");
                        String time = jsonObject.getString("time");
                        JSONArray data = jsonObject.getJSONArray("ret_data");
                        if (data != null) {
    ...
    private List<String> getCatcheTime() {
        ...
        if (!TextUtils.isEmpty(time)) {
            ...
            serviceTimes.add(DateHelper.getStringTime(System.currentTimeMillis()));
        }
        return serviceTimes;
    }
