...
import com.ycbjie.music.executor.online.AbsDownAlbumPic;
...
public class PlayMusicFragment extends BaseFragment implements View.OnClickListener, OnPlayerEventListener {
                ...
                mAdapter.setOnItemClickListener(new OnListItemClickListener() {
                    ...
                    public void onItemClick(View view, int position) {
                        ...
                        if(musicList!=null && musicList.size()>0 && musicList.size()>position && position>=0){
                            BaseAppHelper.get().getMusicService().play(position,AudioBean.Type.NONE);
                            mAdapter.updatePlayingPosition(BaseAppHelper.get().getMusicService());

                           // onChange(musicList.get(position));
    ...
    private void setCoverAndBg(AudioBean music) {
        ...
        if (!albumFile.exists() && !TextUtils.isEmpty(albumMid)) {
           ...
           // downloadAlbumPic(albumMid,albumFile,music);
            new AbsDownAlbumPic(getActivity(), albumMid, albumFile) {
                @Override
                ...
                @Override
                public void onExecuteSuccess(@NonNull String filePath) {
                    music.setCoverPath(filePath);
                    ...
                    LogUtils.e("-----------------------"+ music.getCoverPath());
                    LogUtils.e("========================"+ music.getAlbumId());
                }
    ...
    private void setLrc(final AudioBean playingMusic) {
        if (playingMusic.getType() == AudioBean.Type.LOCAL) {
            ...
            String lrcFileName = FileMusicUtils.getLrcFileName(playingMusic.getArtist(), playingMusic.getTitle());

            String filePath=FileMusicUtils.getLrcDir() + lrcFileName;
            File lrcFile = new File(FileMusicUtils.getLrcDir() + lrcFileName);


            if (lrcFile.exists() && !TextUtils.isEmpty(lrcFile.getPath())) {
                loadLrc(lrcFile.getPath());
                ...
                new AbsSearchLrc(playingMusic) {
                    ...
                    @Override
                    public void onExecuteSuccess(String lrcPath) {
