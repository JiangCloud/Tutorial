...
public class LocalMusicFragment extends BackLazyFragment implements View.OnClickListener {
    ...
    public void initListener() {
        ...
        adapter.setOnItemClickListener(new RecyclerArrayAdapter.OnItemClickListener() {
            ...
            public void onItemClick(int position) {
                List<AudioBean> musicList = BaseAppHelper.get().getMusicLocalList();
                if(musicList!=null && musicList.size()>0 && musicList.size()>position && position>=0){
                    BaseAppHelper.get().getMusicService().play(position,AudioBean.Type.LOCAL);
    ...
    private void getMusicData() {
     ...
     //   music = BaseAppHelper.get().getMusicList();
        music = BaseAppHelper.get().getMusicLocalList();
        if (music.size() > 0) {
            adapter.clear();
