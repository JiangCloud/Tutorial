...
public class OnlineMusicActivity extends BaseActivity implements View.OnClickListener {
    ...
    private static final String MUSIC_LIST_SIZE = "20";
    ...
    private void addHeader(final OnlineMusicList onlineMusicList) {
        ...
        adapter.addHeader(new RecyclerArrayAdapter.ItemView() {
            ...
            public void onBindView(View view) {
                ...
                tv_title.setText(onlineMusicList.getData().getInfo().getTitle());
                tv_update_date.setText(getString(R.string.recent_update, onlineMusicList.getData().getUpdate()));
                tv_comment.setText(onlineMusicList.getData().getInfo().getDesc());
                Glide.with(OnlineMusicActivity.this)
                        .asBitmap()
                        .load(onlineMusicList.getData().getInfo().getPicUrl())
    ...
    private void getData(final int offset) {
        ...
        model.getSongListInfo(OnLineMusicModel.METHOD_GET_MUSIC_LIST,
               Integer.parseInt(type) , MUSIC_LIST_SIZE, String.valueOf(offset))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Consumer<OnlineMusicList>() {
                    @Override
                    public void accept(OnlineMusicList onlineMusicList) throws Exception {
                        if (onlineMusicList == null || onlineMusicList.getData().getList() == null || onlineMusicList.getData().getList().size() == 0) {
                            return;
                        }
                        addHeader(onlineMusicList);
                        adapter.addAll(onlineMusicList.getData().getList());
    ...
    private void setOnLineMusic(OnlineMusicList onlineMusicList) {
        List<OnlineMusicList.SongListBean> song_list = onlineMusicList.getData().getList();

        List<AudioBean> audioBeans=new ArrayList<AudioBean>();
        for (int a=0 ; a<song_list.size() ; a++){

            AudioBean audioBean = new AudioBean();
            audioBean.setMid(String.valueOf(song_list.get(a).getMid()));
            audioBean.setId(String.valueOf(song_list.get(a).getSongId()));
            audioBean.setTitle(song_list.get(a).getTitle());
            audioBean.setArtist(song_list.get(a).getSingerName());
            audioBean.setType(AudioBean.Type.ONLINE);
            audioBean.setAlbumId(song_list.get(a).getId());
            audioBean.setAlbum(song_list.get(a).getAlbumMid());
            audioBean.setDuration(song_list.get(a).getInterval()*1000);
            audioBeans.add(audioBean);

            //audioBean.setDuration(song_list.get(a).getFile_duration()*1000);
        }
       // 加入播放列表
        BaseAppHelper.get().setMusicList(audioBeans);
    }
    ...
    private void showMoreDialog(int position) {
        ...
        dialog.setTitle(adapter.getAllData().get(position).getName());
        String path = FileMusicUtils.getMusicDir() + FileMusicUtils.getMp3FileName(
                onlineMusic.getSingerName(), onlineMusic.getName());
    ...
    private void share(OnlineMusicList.SongListBean onlineMusic) {
        new AbsShareOnlineMusic(this, onlineMusic.getTitle(), onlineMusic.getMid()
                ,onlineMusic.getUrl()) {
            //专辑图片
    ...
    private void lookArtistInfo(OnlineMusicList.SongListBean onlineMusic) {
        ...
        intent.putExtra("artist_id", onlineMusic.getSingerMid());
