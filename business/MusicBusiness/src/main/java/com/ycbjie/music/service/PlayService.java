...
public class PlayService extends Service {
    ...
    public void playPause() {
        if (isPreparing()) {
            ...
            play(getPlayingPosition(),AudioBean.Type.NONE);
        }
    }
    ...
    public void prev() {
        ...
        switch (mode) {
            ...
            case SHUFFLE:
                ...
                play(mPlayingPosition,AudioBean.Type.NONE);
                break;
            //单曲
            case SINGLE:
                play(mPlayingPosition,AudioBean.Type.NONE);
     ...
     * @param position              索引
     */
    public void play(int position, AudioBean.Type playMusicType) {
        if (audioMusics.isEmpty()) {
            return;
        }


        if(playMusicType==AudioBean.Type.LOCAL){

            audioMusics =BaseAppHelper.get().getMusicLocalList();
            BaseAppHelper.get().setMusicList(audioMusics);//加入本地歌曲到列表中
        }
        ...
        // 下载歌词
     /*   String lrcFileName = FileMusicUtils.getLrcFileName(music.getArtist(), music.getTitle());

        String filePath=FileMusicUtils.getLrcDir() + lrcFileName;
        File lrcFile = new File(FileMusicUtils.getLrcDir() + lrcFileName);


        if (!lrcFile.exists() && !TextUtils.isEmpty(music.getMid())) {
            downloadLrc(music.getMid(),filePath);
        }
*/
        // 下载专辑图片
      /*  String albumFileName = FileMusicUtils.getAlbumFileName(music.getArtist(), music.getTitle());
        File albumFile = new File(FileMusicUtils.getAlbumDir(), albumFileName);

        String albumMid =music.getAlbum();//专辑id

        if (!albumFile.exists() && !TextUtils.isEmpty(albumMid)) {


            downloadAlbumPic(albumMid,albumFile,music);
        }
...
*/
        ...
        if(music.getType()==AudioBean.Type.ONLINE){

            getMusicInfo(music);//获取歌曲播放链接
        }else{
    ...
    private void downloadLrc(String songId,String filePath) {
    ...
    @SuppressLint("CheckResult")
    private void downloadAlbumPic(String albumMid, File albumFile,AudioBean  music) {
        OnLineMusicModel model = OnLineMusicModel.getInstance();
        ...
        model.getMusicAlbumInfo(albumMid)
                ...
                .subscribe(new Consumer<Album>() {
                    ...
                    public void accept(Album album) throws Exception {
                        ...
                        OkHttpUtils.downloadFile(getBaseContext(), picUrl,albumFile, new OkHttpUtils.FileCallback() {
                            @Override
                            public void success() {
    ...
    @SuppressLint("CheckResult")
    private void getMusicInfo( AudioBean music) {

        com.yc.httpserver.OkHttpUtils.getWebRequest("http://193.112.117.238:3300/song/urls?id="+music.getMid(), new Callback() {
            ...
            public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
                ...
                LogUtils.d("getMusicPlayUrl============="+s);
                try {
                    JSONObject jsonObject=new JSONObject(s);

                    JSONObject jsonObject1=   jsonObject.getJSONObject("data");

                    LogUtils.d("jsonObject1============="+jsonObject1.toString());
                    if (jsonObject1 == null ||jsonObject1.isNull(music.getMid())) {

                        ToastUtils.showLong("暂时无法播放！");

                        return;
                    }

                    String musicUlr= jsonObject1.getString(music.getMid());
                    LogUtils.d("musicUlr============="+musicUlr);
                    music.setPath(musicUlr);
      ...
      /*  OnLineMusicModel model = OnLineMusicModel.getInstance();
        model.getMusicPlayUrl(OnLineMusicModel.METHOD_DOWNLOAD_MUSIC,music.getMid())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
...
*/
       ...
       /* OnLineMusicModel model = OnLineMusicModel.getInstance();
        model.getMusicDownloadInfo(OnLineMusicModel.METHOD_DOWNLOAD_MUSIC,music.getMid())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
        ...
        new AsyncTask<Void, Void, List<AudioBean>>() {
            ...
            protected void onPostExecute(List<AudioBean> musicList) {
               ...
               /* BaseAppHelper.get().setMusicList(musicList);
                BaseAppHelper.get().setLocalMusicList(musicList);


                if (!BaseAppHelper.get().getMusicLocalList().isEmpty()) {
                    //音频的集合
                    audioMusics = BaseAppHelper.get().getMusicLocalList();
                    //刷新正在播放的本地歌曲的序号
                    ...
                    //获取正在播放的音乐
                    if(mPlayingPosition>=0){
                        mPlayingMusic = BaseAppHelper.get().getMusicLocalList().get(mPlayingPosition);
                    }
                }*/
